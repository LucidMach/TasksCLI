# Uses JSON to store Tasks

Please ensure that `task.json` and `completed.json` are present and initialised with an `[]` inside them

> NOTE: tests don't take console.log output for some reason, passed manual testing of all cases tho

> `tried everything stackoverflow could offer`
